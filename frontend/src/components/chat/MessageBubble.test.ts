...
describe("formatMessageTimestamp", () => {
  it("labels timestamps from today using local time", () => {
    ...
    expect(formatMessageTimestamp(new Date(2026, 6, 26, 2, 18).toISOString(), now)).toMatch(/^Today, /);
  });

  it("labels timestamps from yesterday", () => {
    const now = new Date(2026, 6, 26, 12, 0);
    expect(formatMessageTimestamp(new Date(2026, 6, 25, 22, 30).toISOString(), now)).toMatch(/^Yesterday, /);
  });
