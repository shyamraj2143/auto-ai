  ...
  return Object.fromEntries(
    interaction.fields.map((field) => [
      ...
      field.type === "multiselect"
        ? []
        : field.type === "checkbox" || field.type === "permission"
          ? false
          : ""
    ])
  );
...
function completionMessage(decision: Decision, interaction: IntentInteraction, state: string) {
  ...
  if (decision === "confirm") {
    return `Confirmation saved. AutoAI is preparing the next required step (${readableState}).`;
  }
  return `${interaction.title} saved. AutoAI is preparing the next required step (${readableState}).`;
}

export function DynamicInteractionCard({
  interaction,
  token
}: {
  ...
  const interactionKey = useMemo(
    () =>
      `${interaction.workflow_id || "none"}:${interaction.type}:${interaction.title}:${interaction.fields
  ...
  async function act(decision: Decision) {
    ...
    setError("");
    try {
      const result = await api.submitIntentInteraction(token, interaction.workflow_id, {
        values,
        decision
      });
      if (!result.workflow_id || !result.state) {
        throw new Error("The workflow did not confirm persistence.");
      }
  ...
  async function authenticate() {
    ...
    const secretField = interaction.fields.find(
      (field) => field.type === "otp" || field.type === "secure_password"
    );
    ...
    setError("");
    try {
      const challenge = await api.createSecureChallenge(
        token,
        interaction.workflow_id,
        secretField?.type === "secure_password" ? "password" : "otp"
      );
      const result = await api.submitSecureChallenge(token, challenge.id, secret);
      if (!result.workflow_id || !result.status) {
        throw new Error("The secure workflow did not confirm persistence.");
      }
      setValues(initialValues(interaction));
      setPersistedState(result.status);
      setCompletion(
        "Secure verification accepted for this active workflow. The value was removed from this screen."
      );
      setStatus("complete");
    } catch (cause) {
      setValues(initialValues(interaction));
  ...
  if (status === "complete") {
    return (
      <section
        className="not-prose mt-3 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-4"
        aria-live="polite"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            {persistedState ? (
              <span className="mt-2 inline-flex rounded-full border border-emerald-300/25 px-2 py-1 text-[11px] text-emerald-100">
                {persistedState.replace(/_/g, " ")}
              </span>
            ) : null}
          </div>
        </div>
  ...
  return (
    <section
      className="not-prose mt-3 overflow-hidden rounded-2xl border border-cyan-300/20 bg-slate-950/70"
        ...
        <div className="flex items-start gap-3">
          ...
          <span className="min-w-0 flex-1">
            ...
            {interaction.description ? (
              <p className="mt-1 text-xs leading-5 text-slate-300">{interaction.description}</p>
            ) : null}
          </span>
        </div>
      </header>
      ...
      <div className="p-4">
        {progressOnly ? (
          <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-slate-200">
            ...
            <span>
              AutoAI is working on the current step. This card will update when real progress is
              received.
            </span>
          </div>
        ) : null}
        ...
        <div className="grid gap-3">
          {interaction.fields.map((field) => {
            ...
            const commonClass =
              "w-full min-w-0 rounded-xl border border-white/15 bg-slate-900 px-3 py-2.5 text-sm text-white outline-none transition focus:border-cyan-300/60";
            const label = (
            ...
            if (field.type === "radio") {
              return (
                <fieldset key={field.id} className="grid gap-2 rounded-xl border border-white/10 p-3">
            ...
            if (field.type === "multiselect") {
              const rawSelected = values[field.id];
              const selected: string[] = Array.isArray(rawSelected)
                ? rawSelected.map((item: unknown) => String(item))
                : [];
              return (
                <fieldset key={field.id} className="grid gap-2 rounded-xl border border-white/10 p-3">
                  <legend className="px-1 text-xs font-medium text-slate-200">
            ...
            if (
              ...
              return (
                <label
                  key={field.id}
                  className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-slate-200"
                >
                  <input
                    className="mt-0.5"
                    id={fieldId}
            ...
            if (["file", "camera", "pdf", "image", "signature"].includes(field.type)) {
              ...
              return (
                <label key={field.id} className="grid gap-1.5">
                  {label}
                  <span className="flex items-center gap-2 rounded-xl border border-dashed border-cyan-300/30 bg-cyan-400/[0.04] px-3 py-3 text-xs text-slate-200">
                    <FileUp size={16} />
                    <input
                      id={fieldId}
                      required={field.required}
            ...
            const inputType =
              field.type === "secure_password" || field.type === "otp"
                ? "password"
                : field.type === "phone"
                  ? "tel"
            ...
            return (
              <label key={field.id} className="grid gap-1.5">
                ...
                <input
                  ...
                  inputMode={
                    field.type === "otp" || field.type === "phone" || field.type === "number"
                      ? "numeric"
                      : undefined
                  }
                  autoComplete={
        ...
        {isSecure ? (
          <p className="mt-3 rounded-xl border border-cyan-300/15 bg-cyan-400/[0.04] p-3 text-xs leading-5 text-cyan-50/80">
            Secure values are handled through the protected workflow channel and are not added to
            the visible chat response.
          </p>
        ) : null}
        {error ? (
          <p className="mt-3 flex items-start gap-2 rounded-xl border border-rose-400/20 bg-rose-500/10 p-3 text-xs text-rose-200">
            <AlertTriangle className="mt-0.5 shrink-0" size={15} />
            {error}
          </p>
        ) : null}
        {status === "sending" ? (
          <div className="mt-3 flex items-center gap-2 text-xs text-cyan-100" aria-live="polite">
            <LoaderCircle className="animate-spin" size={15} />
            Saving this step and preparing the next requirement…
          </div>
        ) : null}
        ...
        {interaction.actions.length ? (
          <div className="mt-4 flex flex-wrap gap-2">
            {interaction.actions
              ...
              .map((action) => {
                const primary =
                  action === "submit" || action === "confirm" || action === "authenticate";
                return (
                  <button
                    key={action}
                    type="button"
                    ...
                    onClick={() =>
                      void (action === "authenticate" ? authenticate() : act(action as Decision))
                    }
                    className={
                      primary
                        ? "inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-3.5 py-2.5 text-xs font-semibold text-slate-950 disabled:opacity-50"
                        : "inline-flex items-center gap-2 rounded-xl border border-white/15 px-3.5 py-2.5 text-xs font-medium text-white disabled:opacity-50"
                    }
                  >
                    {actionIcon(action, status === "sending")}
                    {actionLabel(action)}
                  </button>
                );
              })}
