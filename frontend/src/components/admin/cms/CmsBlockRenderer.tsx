...
import { labelCms, lines, namedLines, textValue, type CmsDevice } from "./cmsBlockLibrary";
...
function editableProps(props: RendererProps, block: CmsBlock, key: string) {
  ...
  return {
    ...
    onBlur: (event: FocusEvent<HTMLElement>) => props.onInlineChange?.(block.id, key, event.currentTarget.textContent ?? ""),
...
export function CmsBlockRenderer(props: RendererProps) {
    ...
    return (
      <article className={`cms-render-page cms-render-${props.device ?? "desktop"}`}>
        ...
        <div className="cms-render-body">
          {visible.map((block) => <CmsBlockRenderer {...props} block={block} key={block.id} />)}
        </div>
      </article>
    );
  }
