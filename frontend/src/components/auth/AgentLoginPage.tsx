...
export function AgentLoginPage() {
  ...
  return <div className="auth-page agent-auth-page">
    ...
    <form onSubmit={submit} className="auth-card">
      ...
      <label className="mb-3 block"><span className="mb-1 block text-sm text-slate-200">Agent ID</span><input className="input-dark" value={agentId} onChange={(event) => setAgentId(event.target.value)} autoComplete="username" pattern="[A-Za-z0-9._-]+" minLength={3} required /></label>
      <label className="mb-5 block"><span className="mb-1 block text-sm text-slate-200">Password</span><span className="relative block"><input className="input-dark pr-11" type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" minLength={8} required /><button className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></span></label>
      ...
      <p className="mt-3 text-center text-xs text-slate-500">Forgot your agent password? Contact an administrator to issue a temporary reset.</p>
