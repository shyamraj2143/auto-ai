...
class UserSubscription(Base):
    ...
    plan_id: Mapped[str] = mapped_column(String(32), default="free", index=True, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
    status: Mapped[str] = mapped_column(String(32), default="free", index=True, nullable=False)
    ...
    tokens_added: Mapped[int] = mapped_column(Integer, default=10000, nullable=False)
...
class PlanLimit(Base):
    ...
    max_models: Mapped[int] = mapped_column(Integer, default=0)
...
class PaymentRecord(Base):
    ...
    razorpay_order_id: Mapped[str] = mapped_column(String(120), nullable=True, index=True)
    razorpay_payment_id: Mapped[str] = mapped_column(String(120), nullable=True)
    razorpay_signature: Mapped[str] = mapped_column(String(255), nullable=True)
    paid_at: Mapped[datetime] = mapped_column(DateTime, nullable=True)
    verified_at: Mapped[datetime] = mapped_column(DateTime, nullable=True)
    receipt_number: Mapped[str] = mapped_column(String(64), nullable=True, unique=True, index=True)
    original_amount_paise: Mapped[int] = mapped_column(Integer, nullable=True)
    discount_amount_paise: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    promo_code_id: Mapped[str] = mapped_column(String(36), nullable=True, index=True)
    promo_code_snapshot: Mapped[str] = mapped_column(String(40), nullable=True)
    plan_name_snapshot: Mapped[str] = mapped_column(String(80), nullable=True)
    billing_period_snapshot: Mapped[str] = mapped_column(String(40), nullable=True)
