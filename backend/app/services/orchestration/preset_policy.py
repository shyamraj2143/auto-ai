...
PRESET_POLICIES = {
    ...
    IntelligenceMode.DEEP_RESEARCH: PresetPolicy(IntelligenceMode.DEEP_RESEARCH, frozenset({"groq"}), requires_web_research=True),
    IntelligenceMode.CODING: PresetPolicy(IntelligenceMode.CODING, frozenset({"groq"}), exact_model_count=2),
}
...
def _coding_rank(record: ModelRecord, configured_model: str | None) -> tuple[int, int, float, float, int]:
    ...
    if configured_model and record.actual_model_id == configured_model: family_rank = 0
    elif is_qwen_coder(model_id): family_rank = 1
    elif "qwen" in model_id: family_rank = 2
    elif "coding" in record.capabilities or "coder" in model_id or "code" in model_id: family_rank = 3
    else: family_rank = 4
...
def coding_model_records(records: list[ModelRecord]) -> tuple[ModelRecord | None, ModelRecord | None]:
    candidates = [r for r in _healthy_chat_records(records) if r.provider == "groq"]
    candidates.sort(key=lambda r: _coding_rank(r, _configured_coding_model("groq")))
...
def coding_task_records(records: list[ModelRecord]) -> list[ModelRecord]:
    healthy = [r for r in _healthy_chat_records(records) if r.provider == "groq"]
    healthy.sort(key=lambda r: _coding_rank(r, _configured_coding_model("groq")))
    return healthy[:2]

def coding_configuration_status(records: list[ModelRecord]) -> tuple[bool, str | None]:
    selected = coding_task_records(records)
    return (True, None) if len(selected) >= 2 else (False, "Coding requires two distinct healthy Groq text-chat models.")
