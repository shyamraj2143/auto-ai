...
class GroqService:
    ...
    def _handle_groq_error(exc: GroqError) -> None:
        ...
        if status_code == 429 or re.search(r"\b(rate limit|tpm|tokens per minute|too many requests|request limit)\b", error_text, re.IGNORECASE):
            raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Groq rate limit reached. Please wait a minute, use a smaller prompt, or switch to OpenAI/Gemini.") from exc
    ...
    def _groq_model_candidates(self, model: str, *, web_search: bool) -> list[str]:
        ...
        return list(dict.fromkeys(item for item in [model, settings.GROQ_MODEL, "llama-3.3-70b-versatile", "llama-3.1-8b-instant"] if item))
    ...
    def _raise_provider_error(provider: str, status_code: int, body: str) -> None:
        ...
        try:
            ...
            pass
        raise HTTPException(status_code=status_code if status_code in {400, 401, 403, 429} else status.HTTP_502_BAD_GATEWAY, detail=f"{provider} request failed: {detail}")

    def _openai_payload(self, messages: list[dict[str, Any]], *, model: str, stream: bool = False, max_tokens: int | None = None) -> dict[str, Any]:
        payload: dict[str, Any] = {"model": model, "messages": messages, self._openai_token_parameter(model): max_tokens or settings.GROQ_MAX_TOKENS}
        if stream:
            payload["stream"] = True
    ...
    def _complete_openai(self, messages, *, model, max_tokens=None, request_timeout=None):
        try:
            response = httpx.post(f"{settings.OPENAI_BASE_URL.rstrip('/')}/chat/completions", headers=self._openai_headers(), json=self._openai_payload(messages, model=model, max_tokens=max_tokens), timeout=request_timeout or 90)
    ...
    def _complete_gemini(self, messages, *, model, max_tokens=None, request_timeout=None):
        try:
            response = httpx.post(f"{settings.GEMINI_BASE_URL.rstrip('/')}/chat/completions", headers=self._gemini_headers(), json=self._openai_payload(messages, model=model, max_tokens=max_tokens), timeout=request_timeout or 90)
        except httpx.HTTPError as exc:
    ...
    def _stream_groq(self, messages, *, model, temperature=None):
        try:
            return self._client().chat.completions.create(model=model, messages=messages, temperature=settings.GROQ_TEMPERATURE if temperature is None else temperature, max_tokens=settings.GROQ_MAX_TOKENS, stream=True)
        except GroqError as exc:
            self._handle_groq_error(exc)
    ...
    def analyze_image(self, image_bytes: bytes, filename: str, prompt: str) -> str:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail="Image text recognition is temporarily unavailable.") from exc
