...
class GroqService:
    ...
    def _complete_openai_compatible(self, provider, base_url, headers, messages, *, model, max_tokens=None, request_timeout=None):
        try:
            response = httpx.post(f"{base_url.rstrip('/')}/chat/completions", headers=headers, json=self._payload(messages, model=model, max_tokens=max_tokens), timeout=request_timeout or 90)
        except httpx.HTTPError as exc:
            raise HTTPException(status_code=502, detail=f"{provider} request failed: {exc}") from exc
        if response.status_code >= 400:
            self._raise_http_provider_error(provider, response)
        body = response.json()
    ...
    def complete(self, messages, *, model=None, provider=None, web_search=False, temperature=None, max_tokens=None, request_timeout=None, **_legacy_options):
        ...
        if selected_provider == "openai":
            return self._complete_openai_compatible("OpenAI", settings.OPENAI_BASE_URL, self._openai_headers(), messages, model=selected_model, max_tokens=max_tokens, request_timeout=request_timeout)
        last_error = None
        for candidate in list(dict.fromkeys([selected_model, settings.GROQ_MODEL, "llama-3.3-70b-versatile", "llama-3.1-8b-instant"])):
            try:
                return self._complete_groq(messages, model=candidate, temperature=temperature, max_tokens=max_tokens, request_timeout=request_timeout)
    ...
    def stream(self, messages, *, model=None, provider=None, web_search=False, temperature=None, **_legacy_options):
        ...
        selected_model = self.selected_model(model, provider=selected_provider, web_search=web_search)
        if selected_provider == "openai":
            return self._stream_openai_compatible("OpenAI", settings.OPENAI_BASE_URL, self._openai_headers(), messages, model=selected_model)
        def iterator():
            last_error = None
            for candidate in list(dict.fromkeys([selected_model, settings.GROQ_MODEL, "llama-3.3-70b-versatile", "llama-3.1-8b-instant"])):
                try:
                    yield from self._stream_groq(messages, model=candidate, temperature=temperature)
                    return
                except HTTPException as exc:
                    last_error = exc
                    if exc.status_code in {401,403}:
    ...
    def transcribe_audio(self, audio_bytes: bytes, filename: str, model: str | None = None, *, language: str | None = None, prompt: str | None = None) -> str:
        temp_path = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=Path(filename).suffix or ".webm") as tmp:
    ...
    def _normalize_usage(usage: Any) -> dict[str, int]:
        ...
        prompt = int(usage.get("prompt_tokens", usage.get("inputTokens", 0)) or 0)
        completion = int(usage.get("completion_tokens", usage.get("outputTokens", 0)) or 0)
        total = int(usage.get("total_tokens", usage.get("totalTokens", 0)) or prompt + completion)
