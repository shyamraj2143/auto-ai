...
class DeepResearchService:
    def run(self, messages: list[dict[str, str]], *, payload: ChatRequest, user_id: str) -> DeepResearchResult:
        ...
        if not calls: raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="No deep research provider is configured.")
        ...
        with ThreadPoolExecutor(max_workers=len(calls)) as executor:
            for future in as_completed([executor.submit(self._call_model, call, messages, max_tokens) for call in calls]): results.append(future.result())
        successes = [r for r in results if r.success and r.content.strip()]
        if not successes: raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail="Deep research models could not complete the request.")
    ...
    def model_options(self) -> dict:
        return {"providers": {p: {"enabled": self._provider_configured(p), "models": self._configured_models(p)} for p in ("groq", "openai", "gemini")}, "defaults": {"max_models": settings.DEEP_RESEARCH_DEFAULT_MAX_MODELS, "timeout_seconds": settings.DEEP_RESEARCH_PER_MODEL_TIMEOUT_SECONDS, "final_judge_model": settings.DEEP_RESEARCH_JUDGE_MODEL}}

    def _select_model_calls(self, payload: ChatRequest) -> list[ResearchModelCall]:
        providers = [p for p in (payload.providers or ["groq"]) if p in {"groq", "openai", "gemini"}]
        requested = {"groq": payload.groq_models, "openai": payload.openai_models, "gemini": payload.gemini_models}
        calls: list[ResearchModelCall] = []
        max_models = max(1, min(payload.max_models or settings.DEEP_RESEARCH_DEFAULT_MAX_MODELS, 10))
        for provider in providers:
            if not self._provider_configured(provider): continue
            ...
            for model in selected:
                ...
                if len(calls) >= max_models: return calls
        return calls
    ...
    def _synthesize(self, messages, results, payload, max_tokens):
        ...
        judge_provider = settings.DEEP_RESEARCH_JUDGE_PROVIDER if settings.DEEP_RESEARCH_JUDGE_PROVIDER in {"groq", "openai", "gemini"} else "groq"
        judge_model = payload.final_judge_model or settings.DEEP_RESEARCH_JUDGE_MODEL or self._configured_models(judge_provider)[0]
    ...
    def _configured_models(provider: ResearchProvider) -> list[str]:
        return list(dict.fromkeys({"groq": settings.GROQ_RESEARCH_MODELS, "openai": settings.OPENAI_RESEARCH_MODELS, "gemini": settings.GEMINI_RESEARCH_MODELS}[provider]))

    @staticmethod
    def _provider_configured(provider: ResearchProvider) -> bool:
        key = {"groq": settings.groq_api_key, "openai": settings.OPENAI_API_KEY, "gemini": settings.GEMINI_API_KEY}[provider]
