...
def post_json(url: str, payload: dict[str, object], *, headers: dict[str, str] | None = None, timeout: int = 120) -> dict[str, object]:
    request_headers = {"Accept": "application/json", "Content-Type": "application/json; charset=utf-8"}
    if headers:
        request_headers.update(headers)
    req = request.Request(url, data=json.dumps(payload).encode("utf-8"), headers=request_headers, method="POST")
...
def admin_access_token(base_url: str, email: str, password: str) -> str:
    result = post_json(api_url(base_url, "/auth/login"), {"email": email, "password": password}, timeout=60)
...
def post_notification(base_url: str, payload: dict[str, object], *, secret: str = "", access_token: str = "") -> dict[str, object]:
    ...
    return post_json(api_url(base_url, "/notifications/apk-update"), payload, headers=headers)
...
def main() -> int:
    ...
    parser.add_argument("--admin-email", default=os.getenv("AUTO_AI_ADMIN_EMAIL", ""))
    parser.add_argument("--admin-password", default=os.getenv("AUTO_AI_ADMIN_PASSWORD", ""))
    ...
    try:
        ...
        if not args.secret:
            if not args.admin_email or not args.admin_password:
                print("Provide AUTO_AI_UPDATE_NOTIFY_SECRET or administrator credentials.", file=sys.stderr)
                return 1
            access_token = admin_access_token(args.api_url, args.admin_email, args.admin_password)
            authorization_mode = "administrator"
        result = post_notification(
            args.api_url,
            {"version_code": args.version_code, "version_name": args.version_name, "changelog": args.changelog},
            secret=args.secret,
    ...
    print(json.dumps({**result, "authorization_mode": authorization_mode}, indent=2, sort_keys=True))
    return 0
